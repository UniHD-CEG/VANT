...
cfg = {
    ...
    'VGG11-noBN': [64, 'ReLU', 'DO', 'M', 128, 'ReLU', 'DO', 'M', 256, 'ReLU', 'DO', 256, 'ReLU','DO', 'M', 512, 'ReLU', 'DO',512, 'ReLU', 'DO','M', 512, 'ReLU', 'DO',512, 'ReLU', 'DO','M'],
...
class VGG(nn.Module):
    def __init__(self,
        ...
        self.features, output_channels = self._make_layers(cfg[conf_name], input_shape, dropout_conv)
    ...
    def _make_layers(self, config, input_shape, dropout_conv):
        layers = [
            self._noise_factory.get_noise_operator(),
        ]
        ...
        for x in config:
            if x == 'M':
                ...
                layers += [
                    nn.Dropout(p=dropout_conv), # randomly zero out entire channels 
                ]
            else:
