...
# experiments:
# senstivity: 
#   + inference layer-wise injective noise
#   + dropout layer-wise
# adaptiveness:
#   + train layer-wise injective noise
#   + dropout
# automatic noise distribution:
#   + train with conserve total amount of noise
# Bayesian models:
#   + monte carlo dropout


# types of noises supported
...
class NoiseOperator(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and self.layer_config.enable_in_training:
            ...
            return self._layer_impl(x)
        elif self.training and not self.layer_config.enable_in_training and self.layer_config.GaussStd_in_training:
            return self._layer_impl(x)
        else:
            return x
    ...
    def reset_noise(self, layer_config):
        """
        author: Xiao Wang
        reset the noise configuration, used in incremental training
        only useful to change the GaussStd, scalable
        """
        # ToDo: This should work for all types of configurations
        if isinstance(layer_config, cfg.GaussAddConfig):
            self._layer_impl = impl.AddGaussianNoise(layer_config.GaussMean, layer_config.GaussStd,
                                                     normalize_to_input_size=layer_config.normalize_to_input_size)
            self.layer_config = layer_config
        else:
            raise ValueError(f"The given noise config is not a GaussAddConfig.")
